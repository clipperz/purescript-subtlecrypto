...
main = do
    ...
    runAff_ resolve do
        ...
        do  let genAES mode = do
                    do  k :: ArrayBuffer <- exportKey raw =<< generateKey (aes mode AES.l128) true allUsages
                        log' "  - 128"
                        log' (unsafeCoerce k)
                    do  k :: ArrayBuffer <- exportKey raw =<< generateKey (aes mode AES.l192) true allUsages
                        log' "  - 192"
                        log' (unsafeCoerce k)
                    do  k :: ArrayBuffer <- exportKey raw =<< generateKey (aes mode AES.l256) true allUsages
                        log' "  - 256"
                        log' (unsafeCoerce k)
            log' "- AES-CTR"
            genAES AES.aesCTR
